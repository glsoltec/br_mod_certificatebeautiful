...
class sign_observer {
    ...
    public static function file_created(\core\event\file_created $event) {
        ...
        try {
            ...
            if ($DB->record_exists('certificatebeautiful_sign_log', ['issueid' => $issue->id])) {
                self::$processing = false;
                return;
            }

            $pdfcontent = $file->get_content();
            $signedpdf = pdf\signer\signer::sign_pdf($pdfcontent);
            ...
            debugging("certificatebeautiful sign_observer: {$e->getMessage()}", DEBUG_DEVELOPER);
        }
