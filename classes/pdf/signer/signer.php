<?php
namespace mod_certificatebeautiful\pdf\signer;
...
class signer {
    ...
    public static function sign_pdf(string $pdfcontent): string {
        ...
        if ($pfx === null) {
            throw new \moodle_exception('notconfigured', 'certificatebeautiful');
        }
        $pw = get_config('certificatebeautiful', 'sign_certpassword');
        ...
        $reason = self::pesc(get_config('certificatebeautiful', 'sign_reason') ?: 'Certificate');
    ...
    private static function make_pkcs7(string $data, string $cert, string $pkey, string $pw): string {
        ...
        if ($key === false) {
            ...
            throw new \moodle_exception('errorreadingpfx', 'certificatebeautiful');
        }
        ...
        if (!$r) {
            ...
            throw new \moodle_exception('erroropenssl', 'certificatebeautiful', '', $err ?: '');
        }
        ...
        if ($raw === false || strlen($raw) < 10) {
            throw new \moodle_exception('erroropenssl', 'certificatebeautiful');
        }
    ...
    public static function get_pfx_content(): ?string {
        $fs = get_file_storage();
        $f = $fs->get_area_files(\context_system::instance()->id, 'mod_certificatebeautiful', 'pfxfile', 0, 'id DESC', false);
    ...
    public static function generate_self_signed(string $cn, string $org, string $country, string $password): string {
        ...
        if ($country) { $dn['countryName'] = strtoupper($country); }

        $pk = openssl_pkey_new(['private_key_bits' => 2048, 'private_key_type' => OPENSSL_KEYTYPE_RSA]);
        if ($pk === false) {
            throw new \moodle_exception('erroropenssl', 'certificatebeautiful', '', openssl_error_string());
        }
        ...
        $cert = openssl_csr_sign($csr, null, $pk, 365 * 10, ['digest_alg' => 'sha256']);
        if ($cert === false) {
            throw new \moodle_exception('erroropenssl', 'certificatebeautiful', '', openssl_error_string());
        }

        openssl_pkcs12_export($cert, $pfx, $pk, $password);
        if (empty($pfx)) {
            throw new \moodle_exception('erroropenssl', 'certificatebeautiful', '', 'Failed to export PKCS12');
        }
